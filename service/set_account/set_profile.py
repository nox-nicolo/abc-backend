...
from models.services.service import UserSelectServices
from pydantic_schemas.services.service import ServiceListRequest
from pydantic_schemas.set_account.set_account import (
    SetupProfileRequest,
    UserProfileResponse,
...
async def get_account(user_id: str, db: Session = Depends(get_db)) -> UserProfileResponse:
    ...
    return UserProfileResponse(
        username=user.username,
        pictureUrl=(
            f"{PROFILE_IMAGE_BASE}{user.profile_picture.file_name}"
...
# upload new username and profile picture
async def upload_account_(
    user_id: str,
    data: SetupProfileRequest,
    image_name: UploadFile = File(None),
    db: Session = Depends(get_db),
):
    # NOTE:
    # If SetupProfileRequest is a Pydantic model, replace `data` below with `data.username`
    username_value = data

    await check_username(UsernameCheckRequest(username=username_value), user_id, db)
    ...
    if image_name is not None:
        ...
        if image_name:
            ...
            unique_filename = f"{user.role}/{uuid.uuid4().hex}{extension}"
            r2_path = f"{ImageDirectories.PROFILE_DIR.value}{unique_filename}"

            try:
                upload_file(image_name.file, r2_path, content_type=image_name.content_type)
            except Exception as e:
                raise HTTPException(
    ...
    try:
        ...
        if image_name:
            await image_name.close()


# select default services for the users..
async def selected_(
    selected: ServiceListRequest,
    user_id: str,
    db: Session = Depends(get_db),
):
    user = db.query(User).filter(User.id == user_id).first()
